                ...
                <div class="flex items-center justify-center space-x-8">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="mt-1 text-sm text-slate-600">
						    {{ __('The room') }} <span class="font-bold">{{ $listeningParty->name }}</span> {{ __('is being put together...') }}
                        </p>
                    </div>
                </div>
            </div>
    ...
    @elseif($isFinished)
        <div class="flex items-center justify-center min-h-screen bg-emerald-50">
            <div class="w-full max-w-2xl p-8 mx-8 text-center bg-white rounded-lg shadow-lg">
                <h2 class="mb-4 font-serif text-2xl font-bold text-slate-900">{{ __('This listening party has finished') }} 🥲</h2>
                <p class="mt-2 text-slate-600">{{ __('The room') }} <span class="font-bold">{{ $listeningParty->name }}</span> {{ __('is no longer live.') }}
                </p>
            </div>
        </div>
    @else
        ...
        <div x-show="!isLive" class="flex items-center justify-center min-h-screen bg-emerald-50" x-cloak>
            <div class="relative w-full max-w-2xl p-6 bg-white rounded-lg shadow-lg">
                ...
                <div class="mt-6 text-center">
                    <p class="font-serif font-semibold tracking-tight text-slate-600">{{ __('Starting in:') }}</p>
                    <p class="font-mono text-3xl font-semibold tracking-wider text-emerald-700" x-text="countdownText">
                    </p>
                </div>

                <div class="mt-6">
                    <x-button x-show="!isReady" class="w-full" @click="joinAndBeReady()">{{ __('Join and Be Ready') }}</x-button>
                </div>

                <h2 x-show="isReady" class="mt-8 font-serif text-lg tracking-tight text-center text-slate-900 font-bolder">{{ __('Ready to start the party! Stay tuned.') }} 🫶</h2>
                ...
                <div class="flex items-center justify-end mt-8">
                    <button @click="copyToClipboard();" class="flex items-center justify-center w-auto h-8 px-3 py-1 text-xs bg-white border rounded-md cursor-pointer border-neutral-200/60 hover:bg-neutral-100 active:bg-white focus:bg-white focus:outline-none text-neutral-500 hover:text-neutral-600 group">
                        <span x-show="!copyNotification">{{ __('Share Listening Party URL') }}</span>
                        ...
                        <span x-show="copyNotification" class="tracking-tight text-emerald-500" x-cloak>{{ __('Copied!') }}</span>
                    ...
                    <div class="w-1/2 space-y-6">
                        <div class="p-6 bg-white rounded-lg shadow-lg">
                            ...
                            <div x-show="!isPlaying" class="mt-6">
                                <x-button class="w-full" primary label="{{ __('Join Listening Party') }}" />
                            </div>
                        </div>
                            ...
                            <div class="flex flex-col justify-end flex-1 p-4 overflow-y-auto" id="message-container">
                                <div class="space-y-0.5">
                                    @foreach ($messages as $message)
                                        <div class="px-2 py-2 rounded hover:bg-slate-100" wire:key="{{ $message->id }}">
                            ...
                            <div class="p-4 border-t">
                                @auth
                                    <form class="flex space-x-2" wire:submit='sendMessage'>
                                        <x-input type="text" placeholder="{{ __('Type your message...') }}" wire:model='message'
                                            class="w-full" />
                                        <x-button primary label="{{ __('Send') }}" type="submit" />
                                    </form>
                                @else
                                    <x-button wire:click="authenticateUser" label="{{ __('Login to Chat') }}" class="w-full" />
                                @endauth
                            </div>
                        </div>
                    </div>
